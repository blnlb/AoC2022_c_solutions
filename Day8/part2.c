#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <ctype.h>
#include "../headers/utility.h"

#define GRID_WIDTH 99
#define MAX(A,B) (A ^ ((A ^ B) & -(A < B)))

int **createGrid(int row_length, int col_length);
void destroyGrid(int **grid, int col_length);
int calcScore(int ** grid, int row, int col);
int checkLeft(int **grid, int row, int col);
int checkRight(int **grid, int row, int col);
int checkUp(int **grid, int row, int col);
int checkDown(int **grid, int row, int col);

int main(int argc, char** argv) {
    FILE *fp = fopen("input.txt", "r");
    char* buff = malloc(103);
    
    int **grid = createGrid(GRID_WIDTH, GRID_WIDTH);

    int col, row = 0;
    while (fgets(buff, 103, fp)) {
        for (col = 0; col < GRID_WIDTH; ++col) {
            grid[row][col] = buff[col] - '0';
        }
        ++row;
    }
    fclose(fp);
    free(buff);

    int highScore = 0;
    for (row = 0; row < GRID_WIDTH; ++row) {
        for (col = 0; col < GRID_WIDTH; ++col) {
            highScore = MAX(calcScore(grid, row, col), highScore);
        }
    }

    printf("%d\n", highScore);
    destroyGrid(grid, GRID_WIDTH);
    return 0;
}

int **createGrid(int row_length, int col_length) {
    int **grid = calloc(col_length, sizeof(int*));
    for (int row = 0; row < col_length; ++row) {
        grid[row] = calloc(row_length, sizeof(int));
    }
    return grid;
}

void destroyGrid(int **grid, int col_length) {
    for (int row = 0; row < col_length; ++row) {
        free(grid[row]);
    }
    free(grid);
}

int calcScore(int ** grid, int row, int col) {
    return checkUp(grid, row, col) * checkDown(grid, row, col) *
            checkLeft(grid, row, col) * checkRight(grid, row, col);
}

int checkLeft(int **grid, int row, int col) {
    int result = 1;
    for (int i = col - 1; i > 0; --i) {
        if (grid[row][i] >= grid[row][col]) break;
        else ++result;
    }
    return result;
}

int checkRight(int **grid, int row, int col) {
    int result = 1;
    for (int i = col + 1; i < GRID_WIDTH - 1; ++i) {
        if (grid[row][i] >= grid[row][col]) break;
        else ++result;
    }
    return result;
}

int checkUp(int **grid, int row, int col) {
    int result = 1;
    for (int i = row - 1; i > 0; --i) {
        if (grid[i][col] >= grid[row][col]) break;
        else ++result;
    }
    return result;
}

int checkDown(int **grid, int row, int col) {
    int result = 1;
    for (int i = row + 1; i < GRID_WIDTH - 1; ++i) {
        if (grid[i][col] >= grid[row][col]) break;
        else ++result;
    }
    return result;
}